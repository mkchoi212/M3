...
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self reloadViews];
    UILabel *stockName = [[UILabel alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height/6-30, self.view.frame.size.width, 20)];
    stockName.textAlignment = NSTextAlignmentCenter;
    stockName.text = self.ticker;
    stockName.textColor = [UIColor lightGrayColor];
...
- (UIView *)pagingScrollView:(MHPagingScrollView *)thePagingScrollView pageForIndex:(NSUInteger)index
{
    ...
    reversedArray = [[[graphControllers reverseObjectEnumerator] allObjects]mutableCopy];
	return [reversedArray[index] view];
}
