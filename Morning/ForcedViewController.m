...
- (UIView *)foregroundViewAtIndex:(NSInteger)index scrollView:(PWParallaxScrollView *)scrollView
{
    ...
    if(index == 0){
        ...
        [label setAlpha:1.0f];
        [label setFont:[UIFont fontWithName:@"TimesNewRomanPS-ItalicMT" size:250]];
        [label setUserInteractionEnabled:NO];
    }
    else{
        label = [[UILabel alloc] initWithFrame:CGRectMake(60, 0, screenHeight-120, screenWidth)];
        NSString *t = [[feeds objectAtIndex:index] objectForKey: @"title"];
        label.textColor = [UIColor blackColor];
        [label setFont:[UIFont fontWithName:@"TimesNewRomanPS-ItalicMT" size:250]];
        [label setUserInteractionEnabled:YES];
        label.text = [t uppercaseString];
    }
