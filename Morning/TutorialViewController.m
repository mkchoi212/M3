...
- (void)shimmer{
    ...
    loadingLabel.text = NSLocalizedString(@"today.", nil);
...
-(void)da_panels{
    ...
    MYIntroductionPanel *panel = [[MYIntroductionPanel alloc] initWithimage:[UIImage imageNamed:@"ic"] title:@"Welcome..." description:@"Hey, welcome to Today" ];
    
    //You may also add in a title for each panel
    MYIntroductionPanel *panel2 = [[MYIntroductionPanel alloc] initWithimage:[UIImage imageNamed:@"all"] title:@"All-in-One" description:@"Today contains everything you would want and need to know when you wake up"];
    
    MYIntroductionPanel *panel3 = [[MYIntroductionPanel alloc] initWithimage:[UIImage imageNamed:@"coffee"] title:@"Coffee?" description:@"Today will show you the nearest cafés\nand tell you how to get there"];
    ...
    MYIntroductionPanel *panel6 = [[MYIntroductionPanel alloc] initWithimage:[UIImage imageNamed:@"more"] title:@"More..." description:@"Please accept the privacy authorization requests to enable all features"];
    
    if (self.view.frame.size.height == 480){
    self.introductionView = [[MYIntroductionView alloc] initWithFrame:CGRectMake(0, 30, self.view.frame.size.width, self.view.frame.size.height-55) headerText:@"" panels:@[panel, panel2,panel3,panel4,panel5,panel6] languageDirection:MYLanguageDirectionLeftToRight];
    }
    else{
        self.introductionView = [[MYIntroductionView alloc] initWithFrame:CGRectMake(0, 30, self.view.frame.size.width, self.view.frame.size.height-30) headerText:@"" panels:@[panel, panel2,panel3,panel4,panel5,panel6] languageDirection:MYLanguageDirectionLeftToRight];
    }
        ...
        TWTRLogInButton* logInButton =  [TWTRLogInButton
                                         ...
                                         ^(TWTRSession* session, NSError* error) {
                                             if (session) {
                                                 ...
                                                 notification.notificationLabelBackgroundColor = [UIColor colorWithRed:54.0/255.0 green:152.0/255.0 blue:218.0/255.0 alpha:1.0];
                                                 [notification displayNotificationWithMessage:[NSString stringWithFormat:@"Signed in as %@", [session userName]] forDuration:1.0];
                                                 [self.introductionView hideWithFadeOutDuration:0.3];
                                                 [self introductionDidFinishWithType:MYFinishTypeSwipeOut];
                                             } else {
                                                 NSLog(@"error: %@", [error localizedDescription]);
                                             }
                                         }];
        logInButton.frame = CGRectMake(self.view.center.x-logInButton.frame.size.width/2, self.view.frame.size.height-logInButton.frame.size.height-20, logInButton.frame.size.width, logInButton.frame.size.height);
