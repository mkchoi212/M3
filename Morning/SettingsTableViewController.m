...
-(void)readContents {
    ...
    NSLog(@"self.contents: %@", self.contents[3]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SKSTableViewCell *imagecell = [tableView dequeueReusableCellWithIdentifier:@"image"];
    ...
    if (!imagecell){
        imagecell = [[SKSTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"image"];
    }
    ...
    if(indexPath.row == 3){
        ...
        [imagecell.contentView addSubview:imgView];
        imagecell.expandable = NO;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row == 3){
        ...
        if(temp == 0){
            ...
            [[NSUserDefaults standardUserDefaults]synchronize];
            [[NSNotificationCenter defaultCenter] postNotificationName:notificationName object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"-1", @"pageIndex", nil]];
        }
        NSString *unitChange = @"c";
        if([[NSUserDefaults standardUserDefaults]boolForKey:@"temp"]) {
            unitChange = @"f";
        }
