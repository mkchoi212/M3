...
- (void)updateWeather:(CLPlacemark *)placemark
{
    ...
    NSTimeInterval timesince = lastUpdated.timeIntervalSince1970;
   if(timesince > 60 * 30  || !self.weatherView.hasData || self.changes ){
    ...
    [request performRequestWithHandler:^(id data, NSError *error) {
        if (data) {
            ...
            [request performRequestWithHandler:^(id data, NSError *error) {
                if (data) {
                    self.weatherView.locationLabel.text = [[NSString stringWithFormat: @"%@, %@", city, state] uppercaseString];
                    
                    if([[NSUserDefaults standardUserDefaults]boolForKey:@"temp"]){
                        self.weatherView.currentTemperatureLabel.text = [[NSString stringWithFormat:@"%.0f℉", condition.temperature.f]uppercaseString ];
                        self.weatherView.hiloTemperatureLabel.text = [[NSString stringWithFormat:@"H %.0f  L %.0f", condition.highTemperature.f,condition.lowTemperature.f]uppercaseString ];
                        self.currentUnit = @"f";
                    }
                    else{
                        self.weatherView.currentTemperatureLabel.text = [[NSString stringWithFormat:@"%.0f℃", condition.temperature.c] uppercaseString];
                        self.weatherView.hiloTemperatureLabel.text = [[NSString stringWithFormat:@"H %.0f  L %.0f", condition.highTemperature.c,condition.lowTemperature.c]uppercaseString];
                        self.currentUnit = @"c";
                    }
                    ...
                    self.weatherView.conditionDescriptionLabel.text = [condition.summary uppercaseString];
